...
: Train models on the feature matrix + historical failure records to predict which transformers or line segments are at highest risk in the next 72 hours. Early evidence from IEEE PES studies suggests 20-40% reduction in weather-related unplanned outages with well-trained predictive models.

**Dynamic line rating (DLR)**
: Use `feat_conductor_sag_index` (IEEE 738-2012) to safely increase line capacity during cool, windy conditions, extracting additional capacity without new construction.
...
- **NERC Reliability Standards:** FAC-002, TPL-001 (transmission planning), which require environmental stress analysis in reliability assessments
- **FERC Order 881:** requires utilities to implement ambient-adjusted ratings (AAR) for transmission lines, exactly what `feat_conductor_sag_index` supports

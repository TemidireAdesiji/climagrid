...
def _nasa_payload(lat: float = 31.55, lon: float = -97.15) -> dict:
    ...
    return {
        ...
        "properties": {
            "parameter": {
                "T2M":              dict(zip(hours, [35.0, 36.0, 37.0], strict=False)),
                "WS10M":            dict(zip(hours, [4.2, 5.1, 3.8], strict=False)),
                "ALLSKY_SFC_SW_DWN": dict(zip(hours, [750.0, 820.0, 780.0], strict=False)),
                "RH2M":             dict(zip(hours, [45.0, 42.0, 48.0], strict=False)),
                "PRECTOTCORR":      dict(zip(hours, [0.0, 0.1, 0.0], strict=False)),
            }
        },
    }
